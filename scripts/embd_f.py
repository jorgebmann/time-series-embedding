...
def Graph_embedding(train_sc,valid_sc,test_sc):
    ...
    val_gr=create_vg(val_sc)
    test_gr=create_vg(test_sc)
    return train_gr, valid_gr, test_gr
...
def feature_vector_tda(time_series):
  ...
  # If the filtered diagram is empty, handle this case appropriately
    ...
    # Step 4: Compute the Wasserstein distance and bottleneck distance
...
def create_vg2(data, graph_type):
    all_metrics = []
    for i in range(len(data)):
        if graph_type=='natural':
            vg = ts2vg.NaturalVG(weighted="abs_angle")
...
def TDA_embedding(x_train_df, x_val_df, x_test_df):
    ...
    x_train_a22= np.array(create_vg2(x_train, 'natural'))
    ...
    test_tda=merging_tda(x_test_a1,x_test_a2,x_test_a22,x_test_a3, x_test_a32)
    #we have also added a minmax scaling
    train_tda2,val_tda2,test_tda2=minmax_scaling(train_tda,val_tda,test_tda)
...
def NNCLR_embedding(x_train_df,y_train_df, x_val_df,y_val_df,x_test_df,y_test_df, encoder_model_name,n_classes):
